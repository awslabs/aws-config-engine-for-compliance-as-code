...
rule = __import__('IAM_GROUP_NO_POLICY_FULL_STAR')
...
class ComplianceTest(unittest.TestCase):
    ...
    list_group_policy_names = {'PolicyNames': ['policyname1', 'policyname2']}
    get_group_policy_doc = {'PolicyDocument': '{"Statement": [{"Effect": "Allow", "Action": "*"}]}'}
    no_list_group_policy_names = {'PolicyNames': []}
    ...
    def test_non_compliant_inline(self):
        iam_client_mock.list_group_policies = MagicMock(return_value=self.list_group_policy_names)
        iam_client_mock.get_group_policy = MagicMock(return_value=self.get_group_policy_doc)
        ...
        resp_expected = []
        resp_expected.append(build_expected_response('NON_COMPLIANT', 'AIDAICVB3PKAQMPEGDW2C', annotation='An inline policy attached to the group has full star allow permissions.'))
        assert_successful_evaluation(self, response, resp_expected)

    def test_non_compliant_managed(self):
        iam_client_mock.list_group_policies = MagicMock(return_value=self.no_list_group_policy_names)
        iam_client_mock.list_attached_group_policies = MagicMock(return_value=self.list_attached_policy_arn)
        ...
        response = rule.lambda_handler(lambdaEvent, {})
        resp_expected = []
        resp_expected.append(build_expected_response('NON_COMPLIANT', 'AIDAICVB3PKAQMPEGDW2C', annotation='A managed policy attached to the group has full star allow permissions.'))
        assert_successful_evaluation(self, response, resp_expected)
