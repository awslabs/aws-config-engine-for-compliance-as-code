...
Description:
  Check that all RDS instances are not publically accessible.
...
Feature:
    In order to: to limit the exposure of data
             As: a Security Officer
         I want: To ensure that all RDS instances are not accessible directly via internet.

Scenarios:
    Scenario 1:
      Given: an RDS is in 'Publicly Accessible' mode
       Then: return NON_COMPLIANT

    Scenario 2:
      Given: an RDS is not in 'Publicly Accessible' mode
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    #SCENARIO 1: an RDS is in 'Publicly Accessible' mode
    if is_publicly_accessible:
        return build_evaluation_from_config_item(configuration_item, 'NON_COMPLIANT')

    #SCENARIO 2: an RDS is not in 'Publicly Accessible' mode
